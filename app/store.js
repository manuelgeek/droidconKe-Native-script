  ...
  mutations: {
    ...
    filterSessions(state, { levels, formats, rooms }) {
      Object.keys(state.schedule).forEach((key) => {
        state.schedule[key] = state.schedule[key].filter((e) => e.session_level.includes(levels) && e.session_format.includes(formats) && e.rooms.some((x) => x.title.includes(rooms)));
      });
    },
  },
  actions: {
